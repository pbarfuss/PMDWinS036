 ...
 */
void
MD5Update(MD5_CTX *ctx, const unsigned char *buf, unsigned int len) {
	uint32_t t;
...
static inline void small_memset(void *addr, int c, size_t size) {
    unsigned int i;
 ...
 */
void
MD5Final(MD5_CTX *ctx, unsigned char *digest) {
	...
	*p++ = 0x80;
	...
	if (count < 0) {	/* Padding forces an extra block */
		small_memset(p, 0, count + 8);
		transform(ctx->buf, ctx->in);
		p = (unsigned char *)ctx->in;
		count = 56;
	}
	small_memset(p, 0, count);
